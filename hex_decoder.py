...
from reader import get_bytes_data_from_xml
...
data = get_bytes_data_from_xml()


def decode_fixed(data, p, _from, _to):
    """
    ...
    число должно быть беззнаковым (то есть, только положительным или нулевым). Результат сохраняется в переменную mask.

    _from должен быть больше _to

    fieldBits будет содержать только те биты из  числа data, которые находятся в интервале от _from до _to
    (включительно). Остальные биты будут установлены в 0
    """
    result = {}
...
""" Значение cat (категории) извлекается из первого байта. Это значение используется для определения типа данных.
значение length извлекается из следующих двух байтов  после текущей позиции p. Это значение указывает на длину данных, 
которые будут декодироваться на данной итерации цикла.
Затем p увеличивается на 3 байта, чтобы перейти к следующей порции данных"""
p = 6
...
for DataItem in root.childNodes:
    ...
    if type(DataItem) is not Element:
        continue
    DataItem_id = DataItem.getAttribute("id")
    d[DataItem_id] = {}
        ...
        if DataItemFormats.nodeName == "DataItemFormat":
            for DataItemFormat in DataItemFormats.childNodes:
                ...
                if DataItemFormat.nodeName == "Fixed":
                     ...
                     length = int(Fixed.getAttribute("length"))
                     ...
                     for Bits in Fixed.childNodes:
                         ...
                         for Bit in Bits.childNodes:
                             ...
                             if Bit.nodeName == "BitsShortName":
                                ...
                                for text in Bit.childNodes:
                                    ...
                                    if BitsShortName not in d[DataItem_id]:

                                        d[DataItem_id][BitsShortName] = decode_fixed(data, p, _from, _to)
                         ...
                         if BitsShortName == "spare":
                            BitsPresence = "-1"
                    # VariableId += 1
                         #print("Decoded: ", decode_fixed(data, p, _from, _to))
